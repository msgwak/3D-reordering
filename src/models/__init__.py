...
from src.models.sequence_models.VQ.btst import BTST
from src.models.sequence_models.VQ.teco_btst import TECO_BTST
from src.models.sequence_models.noVQ.btst import BTST_NOVQ
...
def load_ckpt(ckpt_path, replicate=True, return_config=False, 
    ...
    if config.model in ['teco_convS5',  'convS5', 'teco_S5', 'S5', 'convS5_noVQ', 'convLSTM_noVQ', 'teco_BTST', 'BTST']:
        state = TrainState(
            step=state['step'],
            params=state['params'],
...
def get_model(config, need_encode=None, xmap=False, **kwargs):
    ...
    if config.model == 'vqgan':
        ...
        model = partial(TECO_CONVS5,
                        config=config, **kwargs)
    elif config.model == 'BTST':
        model = partial(BTST,
                        config=config, **kwargs)
    elif config.model == 'BTST_noVQ':
        model = partial(BTST_NOVQ,
                        config=config, **kwargs)
    elif config.model == 'teco_BTST':
        model = partial(TECO_BTST,
                        config=config, **kwargs)
