...
def conv_binary_operator(q_i, q_j):
    """Assumes 1x1 kernels
       ...
       :inputs A_i and A_j are (H, W, P)
       :inputs BU_i and BU_j are bszxH_UxW_UxP
       :returns tuple where first entry AA is (H, W, P)
    ...
    AA = A_j * A_i
    A_jBU_i = np.expand_dims(A_j, 0) * BU_i

    return AA, A_jBU_i + BU_j


def apply_convSSM_parallel(A, B_coeff, B, C, us, x0, Q_H, Q_W):
    ...
    Args:
        A (complex64): Conv kernel A                (H, W, P)
    ...
    Bus = np.einsum('zh, yw, lbhwp -> lbzyp', Q_H, Q_W, Bus)
    Bus = Bus * B_coeff[None, None, :]
    #Bus = np.einsum('zyp, zh, yw, lbhwp -> lbzyp', B_coeff, Q_H, Q_W, Bus)
    Bus = Bus.at[0].add(np.expand_dims(A, 0) * x0)

    _, xs = lax.associative_scan(conv_binary_operator, (As, Bus))
    state = xs
    xs = np.einsum('zh, yw, lbhwp -> lbzyp', np.conjugate(Q_H.T), np.conjugate(Q_W.T), xs)
    ys = 2 * vmap_conv(C, xs).real

    return state[-1], ys


def apply_convSSM_sequential(A, B_coeff, B, C, us, x0, Q_H, Q_W):
    """Compute the output sequence of the convolutional SSM
    ...
    def step(x_k_1, u_k):
        ...
        Bu = np.einsum('zh, yw, bhwp -> bzyp', Q_H, Q_W, Bu)
        Bu = Bu * B_coeff[None, :]
        #Bu = np.einsum('zyp, zh, yw, bhwp -> bzyp', B_coeff, Q_H, Q_W, Bu)
        x_k = np.expand_dims(A, 0) * x_k_1 + Bu
        temp = np.einsum('zh, yw, bhwp -> bzyp', np.conjugate(Q_H.T), np.conjugate(Q_W.T), x_k)
        y_k = 2 * lax.conv_general_dilated(temp, C, (1, 1),
                                           'SAME',
