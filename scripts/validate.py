...
def main():
    ...
    if is_master_process:
        ...
        if config.use_wandb:
            wandb.login(key='b2d63bc34c368dc209745b7c984e5d2a55f440e0', relogin=True) # mgwak
            wandb.init(entity='btst', project='BTST', config=config,
                    dir=root_dir)
...
def validate(p_get_eval_metrics, model, iteration, state, test_loader, steps_per_eval, action_conditioned,
    ...
    if is_master_process:
        ...
        print(f"ssim={ssim_val},  lpips={loss_lpips},  psnr={psnr_val}")
