...
def main():
    ...
    if is_master_process:
        ...
        if config.use_wandb:
            ...
            wandb.init(project='ConvS5-CT', config=config, dir=root_dir)
    ...
    data = Data(config)
    prefetch = True
    if 'CT' in config.data_path:
        prefetch = False
    train_loader = data.create_iterator(train=True, prefetch=prefetch)
    print(f"Train batch: {next(train_loader)['video'].shape}")
    test_loader = data.create_iterator(train=False, prefetch=prefetch)
    print(f"Test batch: {next(test_loader)['video'].shape}")
    ...
    if config.model in ["teco_convS5", "convS5", "teco_S5", "S5", "teco_BTST", "BTST"]:
    ...
    elif config.model in ["convS5_noVQ", "convLSTM_noVQ", "BTST_noVQ"]:
        model_par_eval = model(parallel=True, training=False)
        model_seq_eval = model(parallel=False, training=False)
        model = model(parallel=True, training=True)
    ...
    ckpt_dir = osp.abspath(osp.join(config.output_dir, 'checkpoints'))
    ...
    while iteration <= config.total_steps:
        ...
        if iteration > 0 and iteration % config.viz_interval == 0:
            if config.causal_masking:
                if config.use_wandb:
                ...
                if config.order_prediction:
                    # if config.use_wandb: #TODO
                    visualize_order_prediction(iteration, state, test_loader, p_eval_step)
                else:
                    if config.use_wandb:
...
def train(iteration, model, p_train_step, state, train_loader, schedule_fn, rngs):
    ...
    while True:
        ...
        metrics = {k: return_dict[k].mean() for k in metrics}
...
def visualize(model, iteration, state, test_loader, action_conditioned, open_loop_ctx, log_num, p_observe, p_imagine, p_encode, p_decode):
    ...
    if is_master_process:
        wandb.log({'viz/eval_len_{}'.format(log_num+1): wandb.Video(video, fps=20, format='gif')}, step=iteration)
...
def visualize_noVQ(model, iteration, state, test_loader, action_conditioned, open_loop_ctx, log_num, p_observe, p_imagine, p_encode, p_decode, eval_seq_len):
    ...
    if config.model in ["convS5_noVQ", 'convLSTM_noVQ', "BTST_noVQ"]:
...
def visualize_order_prediction(iteration, state, test_loader, p_eval_step):
    batch = next(test_loader)
    frames = batch['video']
    order_real = batch['labels']
    out = p_eval_step(batch, state)
    order_logits = out[1]['order_logits']
    order_predictions = jnp.argmax(order_logits, axis=-1)

    sample_perm_real_one_hot = order_real[0][0]
    sample_perm_real = jnp.argmax(sample_perm_real_one_hot, axis=-1)
    sample_perm_predictions = order_predictions[0][0]

    input_sample_frames = frames[0][0]
    gt_sample_frames = input_sample_frames[sample_perm_real]
    pred_sample_frames = input_sample_frames[sample_perm_predictions]

    # subfigures in two rows, first row is order_real (Axis titlted as GT) and second row is order_predictions (Axis titlted as Reordered)
    fig, axes = plt.subplots(3, len(input_sample_frames), figsize=(len(input_sample_frames), 4), constrained_layout=True)
    for i in range(len(input_sample_frames)):
        axes[0, i].imshow(input_sample_frames[i].squeeze(), cmap='gray')
        axes[0, i].set_title(f'{i}', fontsize=10)
        axes[0, i].axis('off')

        axes[1, i].imshow(gt_sample_frames[i].squeeze(), cmap='gray')
        axes[1, i].set_title(f'{sample_perm_real[i]}', fontsize=10)
        axes[1, i].axis('off')

        c = 'green' if sample_perm_predictions[i] == sample_perm_real[i] else 'red'
        axes[2, i].imshow(pred_sample_frames[i].squeeze(), cmap='gray')
        axes[2, i].set_title(f'{sample_perm_predictions[i]}', fontsize=10, color=c)
        axes[2, i].axis('off')

    # plt.tight_layout()
    # plt.savefig('order_prediction.png')
    if is_master_process:
        wandb.log({'viz/order_prediction': wandb.Image(fig)}, step=iteration)
    plt.close(fig)
...
def validate(p_get_eval_metrics, model, iteration, state, test_loader, steps_per_eval, action_conditioned,
    ...
    for batch_idx, batch in enumerate(tqdm(test_loader, desc="Validating", leave=False, total=steps_per_eval)):
        if batch_idx >= steps_per_eval:
            break
